...
const oldWay = [
  "Hire an agency for $2,000+/month",
  "Wait weeks for content to go live",
  "Only optimized for Google search",
  "AI engines have never heard of you",
  "Miss customer reviews for days",
  "Pay for reports you don't understand",
  "Renew contracts you're locked into",
];

const newWay = [
  "Connect once. Automated forever.",
  "Content goes live on schedule, always",
  "Google AND every major AI engine",
  "ChatGPT, Perplexity, Gemini cite your business",
  "Reviews monitored and flagged instantly",
  "Simple dashboard, real results",
  "Cancel anytime. No contracts.",
];

const comparison = [
  ["$2,000 – $5,000/month", "$97 – $197/month"],
  ["3–6 month contracts", "Cancel anytime"],
  ["Google only", "Google + all AI engines"],
  ["Manual work, slow updates", "Fully automated, 24/7"],
  ["Monthly reports, delayed", "Real-time dashboard"],
  ["You depend on their team", "Set it once, runs forever"],
  ["Optimizes for yesterday", "Built for 2026 and beyond"],
];

const outcomes = [
  { title: "More visibility on Google", desc: "Your Google Business Profile is continuously optimized — posts, photos, hours, reviews, and keywords — so you rank higher in local search results." },
  { title: "Cited by AI engines", desc: "Traffikora builds the structured data and content signals that cause ChatGPT, Perplexity, and Gemini to recommend your business when users ask for local options." },
  { title: "Social media on autopilot", desc: "Content is created and published to your social accounts on a consistent schedule without you writing a single post." },
  { title: "Review management", desc: "Every new review across Google, Yelp, and other platforms is monitored in real time so you never miss customer feedback again." },
  { title: "More traffic, more calls", desc: "Higher visibility on Google and AI engines means more people find your business, visit your website, and call your phone number." },
  { title: "Monthly performance reports", desc: "A clear, simple report shows you exactly what Traffikora has done and what results it has driven — no jargon, just numbers." },
];

const stats = [
  { platform: "Google Search", stat: "8.5B", label: "searches per day", desc: "Traditional SEO — Traffikora has you covered." },
  { platform: "ChatGPT + Perplexity", stat: "100M+", label: "AI searches daily", desc: "The fastest growing discovery channel. Traffikora optimizes for this." },
  { platform: "Gemini + Copilot", stat: "2026", label: "is the tipping point", desc: "AI search is now mainstream. Miss it and you miss customers." },
];

const compRows = comparison.map(([agency, traf]) =>
  `<>
                <div style={{ padding: '18px 32px', borderBottom: '1px solid #eee', borderRight: '2.5px solid #111', background: '#fff' }}>
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#666' }}>✗ ${agency}</p>
                </div>
                <div style={{ padding: '18px 32px', borderBottom: '1px solid #222', background: '#111' }}>
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#fff' }}>✓ ${traf}</p>
                </div>
              </>`
).join("\n              ");

const oldWayRows = oldWay.map(item =>
  `<div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '16px' }}>
                  <span style={{ color: '#cc0000', fontSize: '18px', marginTop: '2px' }}>✗</span>
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#555', lineHeight: 1.5 }}>${item}</p>
                </div>`
).join("\n              ");

const newWayRows = newWay.map(item =>
  `<div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '16px' }}>
                  <span style={{ color: '#E8610A', fontSize: '18px', marginTop: '2px' }}>✓</span>
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#ccc', lineHeight: 1.5 }}>${item}</p>
                </div>`
).join("\n              ");

const statCards = stats.map(s =>
  `<div style={{ border: '2.5px solid #111', padding: '32px', textAlign: 'center' }}>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '13px', fontWeight: 600, letterSpacing: '2px', color: '#E8610A', textTransform: 'uppercase', marginBottom: '8px' }}>${s.platform}</p>
                <p style={{ fontFamily: 'Playfair Display, serif', fontSize: '48px', fontWeight: 900, color: '#111', lineHeight: 1 }}>${s.stat}</p>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', color: '#666', marginBottom: '16px' }}>${s.label}</p>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '15px', color: '#444', lineHeight: 1.5 }}>${s.desc}</p>
              </div>`
).join("\n              ");

const outcomeCards = outcomes.map(o =>
  `<div style={{ borderLeft: '4px solid #E8610A', paddingLeft: '24px' }}>
                <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '20px', fontWeight: 700, color: '#fff', marginBottom: '10px' }}>${o.title}</h3>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '15px', color: '#aaa', lineHeight: 1.7 }}>${o.desc}</p>
              </div>`
).join("\n              ");
...
fs.writeFileSync("C:\\Users\\randy\\traffikfuel\\src\\app\\why-traffikora\\page.tsx", content, { encoding: "utf8" });
console.log("Done");