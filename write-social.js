...
// ============================================
// FIX 1: Social Generator - X label
// ============================================
const socialPath = path.join('C:\\Users\\randy\\traffikfuel\\src\\app\\dashboard\\social\\page.tsx');
let socialContent = fs.readFileSync(socialPath, 'utf8');

socialContent = socialContent.replace(/platform === 'x' \? 'X' :/g, "platform === 'x' ? 'X / Twitter' :");
socialContent = socialContent.replace(/'X'(?=\s*:)/g, "'X / Twitter'");
socialContent = socialContent.replace(/label: 'X'/g, "label: 'X / Twitter'");
socialContent = socialContent.replace(/>X<\//g, ">X / Twitter</");

fs.writeFileSync(socialPath, socialContent, 'utf8');
console.log('FIX 1 DONE - X label updated');

// ============================================
// FIX 2: Billing page - all 4 plans
// ============================================
const billingPath = path.join('C:\\Users\\randy\\traffikfuel\\src\\app\\dashboard\\billing\\page.tsx');

const billingContent = `// @ts-nocheck
'use client'
...
console.log('FIX 2 DONE - Billing page updated with all 4 plans');
