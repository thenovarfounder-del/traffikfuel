  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0a0a', color: '#fff', padding: '40px 32px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', marginBottom: '24px', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {PLATFORMS.map(p => (
                <button key={p} onClick={() => setFilterPlatform(p)} style={{ padding: '6px 14px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500', backgroundColor: filterPlatform === p ? '#f97316' : '#1a1a1a', color: filterPlatform === p ? '#fff' : '#94a3b8' }}>{p}</button>
              ))}
            </div>
          </div>
          <div>
        ...
        <div style={{ backgroundColor: '#111', borderRadius: '8px', border: '1px solid #1f1f1f', marginBottom: '8px', padding: '12px 20px', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 140px', gap: '12px', fontSize: '12px', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase' }}>
        ...
        {loading ? (
          ...
          <div key={post.id} style={{ backgroundColor: '#111', borderRadius: '8px', border: '1px solid #1f1f1f', padding: '16px 20px', marginBottom: '8px', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 140px', gap: '12px', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '13px' }}>{post.platform || '--'}</div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              {post.status !== 'published' && (
                <button onClick={() => markPublished(post.id)} style={{ padding: '5px 10px', borderRadius: '6px', border: 'none', backgroundColor: '#22c55e22', color: '#22c55e', cursor: 'pointer', fontSize: '12px' }}>{publishing === post.id ? '...' : 'Publish'}</button>
              )}
...
console.log('DONE - Content Queue fixed with force-dynamic');