  ...
  return (
    <div className='p-8'>
      ...
      <div className='bg-[#1a1f35] rounded-xl p-6 max-w-2xl'>
        {!hasBrain && <p className='text-red-400 mb-4'>Brain not loaded - run Business Brain first</p>}
        {hasBrain && <p className='text-green-400 mb-4'>Brain loaded</p>}
        <div className='flex items-center gap-4 mb-4'>
          <span className='text-white'>Manual</span>
          <button onClick={() => setIsAuto(!isAuto)} className={w-12 h-6 rounded-full transition-colors }>
            <span className={lock w-5 h-5 bg-white rounded-full transition-transform mx-0.5 } />
          </button>
          <span className='text-white'>Auto</span>
        </div>
        {!isAuto && (
          <input value={topic} onChange={e => setTopic(e.target.value)} placeholder='Enter topic or promotion...' className='w-full bg-[#0f1225] text-white rounded-lg p-3 mb-4 border border-gray-700' />
        )}
