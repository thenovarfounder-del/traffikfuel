  ...
  async function runCheck() {
    ...
    try {
      const response = await fetch('/api/content/citations', {
        ...
        body: JSON.stringify({ query, businessName, userId }),
      })
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <p className="text-gray-400 mb-6">
        Check if AI engines like ChatGPT, Gemini, Perplexity and Claude mention your business when answering relevant questions.
      </p>
      ...
      {loading ? (
        ...
        <>
          <div className="bg-gray-800 rounded-xl p-6 mb-6">
            <label className="block text-sm font-medium text-gray-300 mb-2">Business Name Being Tracked</label>
            <input
              type="text"
              value={businessName}
            ...
            />
            <label className="block text-sm font-medium text-gray-300 mb-2">Query to Test</label>
            <input
              type="text"
              value={query}
            ...
            <div className="mb-6">
              <div className={`border rounded-xl p-6 mb-4 ${scoreBg(checkResult.score)}`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-5xl font-bold ${scoreColor(checkResult.score)}`}>{checkResult.score}%</p>
                    <p className="text-gray-400 text-sm mt-1">Mentioned in {checkResult.mentionCount} of {checkResult.results.length} AI engines</p>
                  </div>
                  <div className="text-right">
                    ...
                    <a href="/dashboard/content/visibility" className="mt-2 inline-block text-blue-400 text-sm hover:text-blue-300">View Visibility Dashboard →</a>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {checkResult.results.map((result) => (
                  <div key={result.engine} className="bg-gray-800 rounded-xl p-5">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <span className="text-white font-semibold">{result.engine}</span>
                        {result.mentioned ? (
                          <span className="bg-green-900/60 text-green-400 text-xs font-semibold px-3 py-1 rounded-full">✅ Mentioned</span>
                        ) : (
                          <span className="bg-red-900/60 text-red-400 text-xs font-semibold px-3 py-1 rounded-full">❌ Not Mentioned</span>
                        )}
                      </div>
                      <button
              ...
              <div className="space-y-3">
                {history.slice(1).map((item, index) => (
                  <div key={index} className="flex items-center justify-between bg-gray-700 rounded-lg px-4 py-3">
                    ...
                    <span className={`font-bold text-sm ${scoreColor(item.score)}`}>{item.score}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {!checkResult && !checking && (
            <div className="bg-gray-800 rounded-xl p-6 text-center text-gray-500">
              ...
              <p className="text-sm mt-2">Test questions your potential clients are asking AI engines right now.</p>
            </div>
          )}
        </>
      )}
