...
export async function POST(request) {
  try {
    ...
    const prompt = 'Write a blog post for a ' + industry + ' business called ' + businessName + ' in ' + city + '. Topic: ' + topic + '. Tone: ' + tone + '.'
      ...
      + ' Return ONLY a JSON object with fields: title, metaDescription, content (full HTML), wordCount. No markdown. No backticks.'
    ...
    if (!response.ok) {
      return Response.json({ success: false, error: JSON.stringify(data) }, { status: 500 })
    }

    if (!data.content || !data.content[0]) {
      return Response.json({ success: false, error: 'No content returned: ' + JSON.stringify(data) }, { status: 500 })
    }
