...
export default function GlossaryPage() {
  ...
  return (
    <main suppressHydrationWarning className='min-h-screen bg-[#111111] text-white'>
      {/* Hero */}
      <section className='pt-28 pb-16 px-6 text-center' style={{ background: 'linear-gradient(135deg, #111111 0%, #1a1a1a 50%, #111111 100%)' }}>
        <p className='text-[#E8610A] font-semibold uppercase tracking-widest text-sm mb-4'>Reference Guide</p>
        <h1 className='font-playfair text-4xl md:text-6xl font-bold text-white mb-6 leading-tight'>
          AI &amp; SEO Marketing<br /><span className='text-[#E8610A]'>Glossary</span>
        </h1>
        <p className='text-gray-300 text-lg md:text-xl max-w-2xl mx-auto mb-10'>
          Every term you need to understand AI marketing, local SEO, and generative engine optimization -- explained in plain English.
        </p>
        {/* Letter Nav */}
        <div className='flex flex-wrap justify-center gap-2 max-w-3xl mx-auto'>
          {letters.map((l) => (
            <a key={l} href={'#letter-' + l} className='w-9 h-9 rounded-full border border-[#E8610A] text-[#E8610A] flex items-center justify-center text-sm font-bold hover:bg-[#E8610A] hover:text-white transition-colors'>
              {l}
            </a>
          ))}
        </div>
      ...
      <section className='max-w-4xl mx-auto px-6 pb-24'>
        {terms.map((group) => (
          <div key={group.letter} id={'letter-' + group.letter} className='mb-14'>
            <div className='flex items-center gap-4 mb-6'>
              <span className='text-5xl font-playfair font-bold text-[#E8610A]'>{group.letter}</span>
              <div className='flex-1 h-px bg-[#E8610A] opacity-30'></div>
            </div>
            <div className='space-y-6'>
              {group.entries.map((entry) => (
                <div key={entry.term} className='border border-white border-opacity-10 rounded-xl p-6 hover:border-[#E8610A] hover:border-opacity-50 transition-colors'>
                  <h2 className='text-lg font-bold text-white mb-2'>{entry.term}</h2>
                  <p className='text-gray-300 leading-relaxed'>{entry.definition}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* CTA */}
      <section className='py-20 px-6 text-center' style={{ background: 'linear-gradient(135deg, #E8610A 0%, #C84E06 100%)' }}>
        <h2 className='font-playfair text-3xl md:text-5xl font-bold text-white mb-4'>Put These Strategies to Work</h2>
        <p className='text-white text-opacity-90 text-lg mb-8 max-w-xl mx-auto'>Traffikora handles AEO, local SEO, social media, and blog automation -- all on autopilot.</p>
        <a href='/signup' className='inline-block bg-white text-[#E8610A] font-bold text-lg px-10 py-4 rounded-full hover:bg-gray-100 transition-colors'>
          Start Free Trial
        </a>
      </section>
    </main>
